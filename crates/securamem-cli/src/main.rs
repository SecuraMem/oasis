...
async fn main() -> securamem_core::Result<()> {
    ...
    match cli.command {
        ...
        Commands::GenVendorKeys => {
            ...
            println!("✓ Keys saved to vendor_private.pem and vendor_public.pem");
            println!("⚠️  Update securamem-core/src/license.rs with the content of vendor_public.pem!");
        ...
        Commands::Index { path } => {
            ...
            if path.is_dir() {
                ...
                println!("✓ Successfully indexed directory {:?}", path);
            } else {
                ...
                println!("✓ Successfully indexed {:?}", path);
            }
